<template>
  <div class="demo-wrapper">
    ...
    <div class="control-panel">
      ...
      <select v-model="selectedId" class="control-select" :disabled="objects.length === 0">
      ...
      <span class="panel-value">{{ Number(px).toFixed(1) }}</span>
      ...
      <span class="panel-value">{{ Number(pz).toFixed(1) }}</span>
      ...
      <span class="panel-value">{{ Number(ry) }}°</span>
      ...
      <span class="panel-value">{{ Number(s).toFixed(1) }}</span>
    </div>
  </div>
</template>
...
function makeMaterial(color) {
  return new THREE.MeshStandardMaterial({ color, roughness: 0.45, metalness: 0.25, wireframe: false })
}

function addObject(object, name) {
  const id = String(nextId)
  object.userData.__editorId = id
  objects.value.push({ id, name: `${name}-${id}`, object })
  nextId++
  scene.add(object)
  selectedId.value = id
}
...
function syncControlsFromSelection() {
  ...
  wireframe.value = !!mats.filter(Boolean)[0]?.wireframe
}
...
function applyControlsToSelection() {
  const obj = selectedObject.value
  if (!obj) return
  obj.position.x = Number(px.value) || 0
  obj.position.z = Number(pz.value) || 0
  obj.rotation.y = THREE.MathUtils.degToRad(Number(ry.value) || 0)
  obj.scale.setScalar(Math.max(0.001, Number(s.value) || 1))
  applyWireframe(obj)
...
function onPointerDown(e) {
  ...
  const hits = raycaster.intersectObjects(objects.value.map((o) => o.object), true)
...
function init() {
  ...
  grid = new THREE.GridHelper(60, 30, 0x222244, 0x111122)
  scene.add(grid)
